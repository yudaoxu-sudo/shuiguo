...
- 抖音门店分项短暂高于本月总额时，前两轮保持严格重读；最终差额同时不超过 500 元和总额 0.5% 才记录负向“平台同步差额”，并在报表中提示
...
`login-healthcheck` 和 `report-healthcheck` 遇到页面加载、网络超时等可恢复异常时，沿用 20 分钟静默复验与单次告警。芝麻地登录失效会进入独立的 3 小时静默修复窗口：立即尝试一次，随后在第 5、15、30 分钟重试，之后每 30 分钟重试；期间不发送验证码图、登录故障提示或自动恢复提示，且等待时不占用浏览器锁。3 小时后最终仍未恢复时，只发送一次验证码和回复提示，后续检查不会重复发送。两条健康检查链路与夜间补跑共用同一个持久修复事件，不会重置截止时间；无法自动处理的其他错误仍按原告警规则处理。`report-healthcheck` 使用 `NO_DINGTALK=1` 做真实抓取预检，不推送正式报表。双报告日期会同时预检聚合接口版和网页版。日报、登录检查和手动芝麻地登录共用 `output/browser-profile.lock`，避免多个浏览器进程同时抢同一个登录态目录。
...
芝麻地登录过期时，系统先在后台静默自动修复。持续 3 小时仍未恢复后才发送一次验证码图；回复 `验证码ABCD` 即可恢复服务器登录态。主动发送 `登录` 仍可立即进入人工修复。
...
需要由常驻监听器在既有钉钉会话中收取短信码时，显式运行：

```bash
pnpm douyin:login:repair
```

运行前 5 分钟内，先由接收验证码的人在目标钉钉会话发送 `准备抖音短信`，让监听器保存本次会话与发送人上下文。该命令会在抖音页面发送短信前检查上下文，并把修复请求绑定到当时的会话和发送人；请求和会话上下文超过 5 分钟会失效，监听器只接受绑定会话及发送人的回复。定时报告和健康检查不会自行执行这个真实登录动作。
